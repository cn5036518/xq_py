...
class Account:
    ...
    def operate_file(self,username,password):  #0 读写文件--判重等文件操作  参数是用户名和密码
        self.f1 = open('acount.txt', mode='a+', encoding='utf-8')  # 先w+  后a+   可追加可读
        # 1 新建文件，文件对象存储成成员变量，方便方法间使用；追加后，光标默认在文件末尾
    ...
    def register(self): #1 注册功能
        ...
        self.operate_file(username,password)  #调用成员方法，检查用户名是否已经存在-判重
...
面向对象：
1、把文件读写单独分拆出来，封装成一个成员方法--封装思想
   读写文件和用户名判重（检查用户名是否已经存在）
   好处是：
        分拆和封装成成员方法后，要进行文件读写的话，直接调用该成员方法即可，更加灵活
2、注册的成员方法调用文件读写的成员方法
'''
