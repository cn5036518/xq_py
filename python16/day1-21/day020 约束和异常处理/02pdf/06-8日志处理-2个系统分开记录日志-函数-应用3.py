...
#多个系统分别记录不同的日志文件
def multi_system_log(filename,system,message):
    #参数1   日志文件名
    #参数2   系统名字
    #参数3   堆栈的报错信息
    ...
    # 7、日志对象调error写入提示信息
    # logger1.error(system)  # 格式化   #可以注释掉   主要用于标识系统
    logger1.error(message)  #把堆栈的报错信息写入到日志文件
...
for i in range(3):
    try:
        ...
        print(val)  #输出到控制台
        # multi_system_log('f1.log', '系统1',val)
        ...
        print(val)
    # logger1.error(val)
    multi_system_log('f1.log', '系统1', val)  #调函数
print('----------------------------------------1   函数 应用3  系统2')
...
for i in range(2):
    try:
        ...
        # print(val)
    # logger1.error(val)
    multi_system_log('f2.log', '系统2', val)   #调函数
print('----------------------------------------2   函数 应用3  系统2')
