...
#二、接口类
class Interface1:
    def test(self,url,params1,headers1,method1,data_type1,check_point1,yes_no1,i,path2_case,path1_report,sh1,wb1):
        ...
        wb1.save(path1_report)   #另存为到path1--就是一个测试报告的性质（填写了测试结果和响应数据后）


#三 获取excel字段类-遍历case类
class For_case1:
    def for_case1(self,path2_case,path1_report):
        # 2 获取工作簿
        wb1 = openpyxl.load_workbook(path2_case)
        ...
        for i in range(max_row1-1):  #0,1
            ...
            i1 = Interface1()
            i1.test(url, params1, headers1, method1, data_type1, check_point1, yes_no1, i+2,path2_case,path1_report,sh1,wb1)
