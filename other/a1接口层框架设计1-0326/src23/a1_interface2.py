...
#四、读取excel
#1指定excel的路径
path1 = r"D:\PycharmProjects\xiaoqiang\base_181027\a1接口层框架设计1-0326\src23\testcase2.xlsx"
...
#三、接口类
class Interface1:
    def __init__(self,url,params1,headers1,method1,data_type1,check_point1,yes_no1,i):
        self.url = url
    ...
    def test(self):
        ...
        if ret1 == ret2:
            ...
            sh1.cell(self.i+2,11).value = "通过"  #通过i+2 保证读取的行和写入的行是同一行
        else:
            print("测试不通过")
            sh1.cell(self.i+2, 11).value = "不通过"
        result1 = json.dumps(result1,ensure_ascii=False)  #字典转换成json字符串，参数2是显示中文的
        sh1.cell(self.i+2, 12).value = result1
        wb1.save(path1)
...
#4 获取单元格
    def for_case1(self):
        for i in range(max_row1-1):  #0,1
            ...
            i1 = Interface1(url, params1, headers1, method1, data_type1, check_point1, yes_no1, i+2)
            Interface1.test(self)
