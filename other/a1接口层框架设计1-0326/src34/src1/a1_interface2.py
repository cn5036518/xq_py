...
class Interface1:
    def test(self,url,params1,headers1,method1,data_type1,check_point1,yes_no1,i,path1_report,sh1,wb1):
        ...
        if ret1 == ret2:
            ...
            sh1.cell(row=i,column=11).value = "通过"  #通过i+2 保证读取的行和写入的行是同一行
        else:
            print("测试不通过")
            sh1.cell(row=i, column=11).value = "不通过"
...
class For_case1:
    def for_case1(self,path2_case,path1_report):  #参数1-测试用例路径 参数2-测试报告路径
        ...
        for i in range(max_row1-1):  #0,1
            ...
            url1 = sh1.cell(row=i+2,column=3).value.replace('\n','').replace('\r','').strip()
            #python3.3.3版本不支持（田电脑） python3.3.5版本才支持(我家里电脑)
            # url1 = sh1.cell(row=i+2,column=3).value  #python3.3.3版本只支持这个写法（田电脑）
            # print(url1)
            # 会报错 AttributeError: 'int' object has no attribute 'upper'

            url2 = sh1.cell(row=i+2,column=4).value.replace('\n','').replace('\r','').strip()
            url = url1 + url2

            method1 = sh1.cell(row=i+2,column=5).value.replace('\n','').replace('\r','').strip()

            data_type1 = sh1.cell(row=i+2,column=6).value.replace('\n','').replace('\r','').strip()
            #这里是去掉从excel读取后的字段的两端的空格，以及回车换行等

            params1 = sh1.cell(row=i+2,column=7).value.replace('\n','').replace('\r','').strip()
            # params1 = json.loads(params1)  #方式1：从excel获取的是json字符串，转成python字典，放入内存
            params1 = eval(params1)  #方式2：从excel获取的是json字符串，
            # print(type(params1))


            check_point1 = sh1.cell(row=i+2,column=8).value.replace('\n','').replace('\r','').strip()
            check_point1 = json.loads(check_point1)  # 从excel获取的是json字符串，转成python字典，放入内存

            yes_no1 = sh1.cell(row=i+2,column=10).value.replace('\n','').replace('\r','').strip()
            # print(yes_no1)
            if yes_no1 == "否":
